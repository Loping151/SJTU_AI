...
class Louvain:
    ...
    def delta_modularity(self, node: int, community: Community) -> float:
        ...
        k_iin = community.node2comm_degree(node)
        k_i_in = self.G.get_in_degree(node)
        k_i_out = self.G.get_out_degree(node)
        Sigma_in = community.get_in_degree()
        Sigma_out = community.get_out_degree()
        delta_q = k_iin / self.G.M  - (k_i_out * Sigma_in + k_i_in * Sigma_out) / (self.G.M ** 2)
    ...
    def phase2(self):
        ...
        for metanode in self.G.nodes:
            current_community_id = self.get_community_of_node(metanode).id
            for neighbor in self.G.get_out_neighbors(metanode):
                neighbor_community_id = self.get_community_of_node(neighbor).id
                add_weight = self.G.get_edge_weight(metanode, neighbor)
                new_edges[(current_community_id, neighbor_community_id)] += add_weight
    ...
    def merge_communities(
        ...
        # v1: greedy merge
        # v2: greedy merge with similarity heap
        # v3: use in_degree and out_degree to calculate similarity
        # v4: use in_degree and out_degree to calculate similarity, and use greedy merge at last
        # it's fking hard.
        from collections import defaultdict
        import heapq

        community_to_nodes = defaultdict(set)
        for node, community in node2commid.items():
            community_to_nodes[community].add(node)

        community_in_degree = defaultdict(int)
        community_out_degree = defaultdict(int)
        for node in node2commid:
            community = node2commid[node]
            community_in_degree[community] += self.G.get_in_degree(node)
            community_out_degree[community] += self.G.get_out_degree(node)
        ...
        for i in range(len(communities)):
            for j in range(i + 1, len(communities)):
                comm1 = communities[i]
                comm2 = communities[j]
                in_diff = abs(community_in_degree[comm1] - community_in_degree[comm2])
                out_diff = abs(community_out_degree[comm1] - community_out_degree[comm2])
                similarity = -(in_diff + out_diff)
                heapq.heappush(similarity_heap, (similarity, comm1, comm2))

        while len(community_to_nodes) > n_expected_communities:
            _, comm1, comm2 = heapq.heappop(similarity_heap)
            if comm1 in community_to_nodes and comm2 in community_to_nodes:
                community_to_nodes[comm1].update(community_to_nodes.pop(comm2))
                for node in community_to_nodes[comm1]:
                    node2commid[node] = comm1
                community_in_degree[comm1] += community_in_degree.pop(comm2)
                community_out_degree[comm1] += community_out_degree.pop(comm2)

            if len(community_to_nodes) == n_expected_communities + 1:
                similarity_heap = []
                communities = list(community_to_nodes.keys())
                for i in range(len(communities)):
                    for j in range(i + 1, len(communities)):
        ...
        key_dict = {k:idx for idx, k in enumerate(community_to_nodes.keys())}
        node2commid = {node:key_dict[node2commid[node]] for node in node2commid}
