...
class KMeans:
    ...
    def calc_radius(self, X: np.ndarray, labels: np.ndarray, centroids: np.ndarray):
        ...
        radius = np.zeros(self.n_clusters)
        ...
        for k in range(self.n_clusters):
            ...
            if len(points_k) > 0:
                ...
                assert len(distances) == len(points_k)
                radius[k] = np.max(distances)

        assert radius.shape == (self.n_clusters,)
        # End of TODO
    ...
    def cluster(
        ...
        for _ in range(max_iter):
            ...
            if np.linalg.norm(centroids - old_centroids) < eps:
                break
            # the labels are returned outside. Why remember?
        # End of TODO
