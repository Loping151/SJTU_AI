...
from model.bert import Bert_WMM
...
if args.aug:
    train_path = os.path.join(args.dataroot, 'train_augment_asr.json')
else:
...
def set_optimizer_bert(model, args):
    optimizer = Adam([
    {'params': model.bertmodel.parameters(), 'lr': 1e-5},  
    {'params': model.output_layer.parameters(), 'lr': 5e-4}  
    ])
    return optimizer
...
if not args.testing:
    ...
    optimizer = set_optimizer_bert(model, args)           ################################
