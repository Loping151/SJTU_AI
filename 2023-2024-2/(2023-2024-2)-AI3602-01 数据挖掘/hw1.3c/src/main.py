...
for i in range(0, numDocs):
    ...
    for index in range(len(words) - 2):
        ...
        shingleHash = int.from_bytes(sha1(shingle.encode('utf-8')).digest(), 'little') % (2**32) # treat as 32-bit integer
        shinglesInDoc.add(shingleHash)
    ##### end of TODO #####
...
for docID in docNames:
    ...
    for i in range(numHashes):
        ...
        for shingleID in shingleIDSet:
            hashCode = (a_list[i] * shingleID + b_list[i]) % nextPrime
...
for i in range(0, numDocs):
    ...
    for j in range(i + 1, numDocs):
        ...
        estJ = sum(1 for k in range(numHashes) if signature1[k] == signature2[k]) / numHashes
        estJSim[getTriangleIndex(i, j)] = estJ
        ##### end of TODO #####
