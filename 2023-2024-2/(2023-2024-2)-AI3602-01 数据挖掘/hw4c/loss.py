...
class SimilarityLoss(nn.Module):
    ...
    def forward(
        ...
        pos_sim = nn.functional.cosine_similarity(code_features, pos_desc_features, dim=-1, eps=self.eps)
        neg_sim = nn.functional.cosine_similarity(code_features, neg_desc_features, dim=-1, eps=self.eps)

        loss = torch.clamp(self.margin - pos_sim + neg_sim, min=0.0).mean()
