...
class Community:
    ...
    def node2comm_in_degree(self, node: int) -> int:
        ...
        d_in = sum(self.graph.get_edge_weight(neighbor, node) \
                   for neighbor in self.graph.get_in_neighbors(node) \
                   if neighbor in self.nodes) # It's wired that it can be finished within one line instead of 5
    ...
    def node2comm_out_degree(self, node: int) -> int:
        ...
        d_out = sum(self.graph.get_edge_weight(node, neighbor) \
                    for neighbor in self.graph.get_out_neighbors(node) \
