...
def config():
    parser = argparse.ArgumentParser(description='Train JRNN')
    ...
    parser.add_argument('--rnn_type', type=str, default='lstm', help='lstm or rnn')
...
def test(args, test_loader):
    model = JRNN(args.rnn_type, 28, 128, 3, 10)
    model.load('logs/' + args.exp_name + '/jrnn.pkl')
    test_acc = 0
...
def train(args):
    ...
    lr_scheduler = jt.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.1)
    ...
    print('\n training completed.')
    model.save('logs/' + args.exp_name + '/jrnn.pkl')
    test(args, test_loader)
